    ...
    while not done:
        ...
        for y in range(TrailMap.shape[0]):
            for x in range(TrailMap.shape[1]):
                if FoodMap[y, x] > 0:
                    value = FoodMap[y, x]
                    pygame.draw.circle(screen, (33, min(255, value + 33), 33), (x, y), 0, 0)
                elif FoodMap[y, x] < 0:
                ...
                if TrailMap[y, x] >= 8:
                    ...
                    if TRAIL:
                        ...
                        if value > 255:
                            value = 255
                        pygame.draw.circle(screen, (max(CL[0], value - 100), max(CL[1], value - 100), max(CL[2], value - 100)), (x, y), 0, 0)
        ...
        pygame.image.save(screen, 'output/file-' + str(N) + '.png')
        N += 1   
        pygame.display.update()
        screen.fill(CL)
